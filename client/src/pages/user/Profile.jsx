...
const Profile = () => {
  ...
  const handleFollow = async () => {
    ...
    try {
      await axios.post(
        `${USER_API_URL}/follow/${profile._id}`,
        {},
        { withCredentials: true }
      );
    } catch (err) {
            ...
            {isMyProfile ? (
              ...
              <>
                <button
                  ...
                  className={`${
                    isFollowing ? "bg-gray-700" : "bg-sky-500"
                  } text-white font-semibold py-1 px-4 rounded`}
                >
                  {isFollowing ? "Following" : "Follow"}
                </button>
                <button className="bg-gray-700 text-white font-semibold py-1 px-4 rounded">
                  Message
                </button>
              </>
            )}
          ...
          {posts.map((post) => (
            <Link to={`/post/${post._id}`} key={post._id}>
              <div className="relative aspect-square group">
                <img
                  src={post.image.url} // Use post.image.url if you updated the model
                  alt="User post"
